...
import {taxClassification} from './data/dropdownListData';

const renderItem = ({item}) => {
  return (
    <View style={{width: wp('80'), marginTop: 4}}>
      <Text
        numberOfLines={2}
        style={{
          width: '100%',
...
const theme = '#21D17F';
const border = '#BDDBE8';
const customTitleDropdown = ({item, value, label, placeHolder, getValue}) => {
  ...
  const fillItems = () => {
    ...
    if (value.length > 0) {
      for (let i = 0; i < value.length; i++) {
        tempArray.push({id: value[i], title: value[i]});
      }
      setList(tempArray);
      // for (let i = 0; i < tempArray.length; i++) {
      //   tempGoodFor.push(tempArray[i].id);
      // }
      // getValue(tempGoodFor);
    }
  };
  ...
  const removeItem = id => {
    ...
    for (let i = 0; i < tempList.length; i++) {
      temListArr.push(tempList[i].id);
    }
    console.log(temListArr);
    getValue(temListArr);
  };
      ...
      {active ? (
        <ScrollView
          ...
          <CheckboxList
            theme={theme}
            listItems={
              loader ? [] : item == undefined ? taxClassification : item
            }
            onChange={item => {
              onMethodClick(item);
            }}
...
export default customTitleDropdown;
...
const styles = StyleSheet.create({
  ...
  headingStyle: {
    ...
    marginBottom: Platform.OS == 'android' ? 10 : 13,
  },
  subheadingStyle: {
    width: wp('80'),
    color: '#707070',
