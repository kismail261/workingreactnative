...
import { EquipmentType } from '../components/data/equipmentType'
import { MyTextinputMultiline } from '../../../common/textinputMultiline'
import Btn from '../../../common/meduimBtnSP'
import { editServiceFacilities } from '../../../redux/listing/listing.action'
...
const facilities = (props) => {
    ...
    const [facilitiesRes, setFacilitiesRes] = useState([])
    const [amentiesRes, setAmentiesRes] = useState([])
    const [equipmentRes, setEquipmentRes] = useState([])
    ...
    const getListingDetail = () => {
        ...
        axios.get(`${Url}api/listing/by/${recordId}`, { headers: headers })
            .then(resp => {
                ...
                setPreparation(response.instructions)
                setAmentiesRes(response.amenities)
                setEquipmentRes(response.equipment)
                setFacilitiesRes(response.facilites)
    ...
    const getEquipment = selectedItems => {
        setEquipment(selectedItems)
    };
    ...
    const addFacilities = () => {
        if (facilitiesRes.length == 0) {
            showMessage("Should select facilities", 'warning')
        } else if (amentiesRes.length == 0) {
            showMessage("Should select amenties", 'warning')
        } else if (equipmentRes.length == 0) {
            showMessage("Should select equipment", 'warning')
        } else if (preparation == '') {
            ...
            dispatch(editServiceFacilities(token, recordId, data, props.navigation, 3))
        }
    }
            ...
            <View style={styles.container} >
                ...
                <ScrollView contentContainerStyle={{ width: wp('100'), alignItems: 'center', paddingBottom: 20 }} >
                    <View style={styles.subContainer} >
                        ...
                        <Text style={styles.subHeading} >Please tell us what Amenities and Equipment is available</Text>
                        <CustomTitleDropdown item={Facilities} value={facilitiesRes} label="What Facilities are used for this facility?" placeHolder="Select Facilities" getValue={getFacilities.bind(this)} />
                        <CustomTitleDropdown item={AmenitiesType} value={amentiesRes} label="What Amenities are available?" placeHolder="Select Amenities" getValue={getAmenties.bind(this)} />
                        <CustomTitleDropdown item={EquipmentType} value={equipmentRes} label="What Equipment is available and included at this facility?" placeHolder="Select Equipment" getValue={getEquipment.bind(this)} />
