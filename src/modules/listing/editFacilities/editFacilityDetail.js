...
import { editFacilityDetails } from '../../../redux/listing/listing.action'
...
const addFacilites = (props) => {
    ...
    const getListingDetail = () => {
        ...
        axios.get(`${Url}api/listing/by/${listId}`, { headers: headers })
            .then(resp => {
                ...
                setWhatsIncluded(response.specialConditions);
    ...
    const uploadServiceDetail = () => {
        if (selectSport == 'select') {
            ...
            showMessage("Should select facility type", 'warning')
        } else if (serviceName == '') {
            showMessage("Facility title field should not be blank", 'warning')
        } else if (serviceDesc == '') {
            showMessage("Facility Description field should not be blank", 'warning')
        } else if (whatsIncluded == '') {
            showMessage("Special conditions field should not be blank", 'warning')
        } else {
            dispatch(setLoader(true))
            let data = {
            ...
            dispatch(editFacilityDetails(token, data, listId, props.navigation))
