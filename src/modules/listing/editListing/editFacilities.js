...
import { Animated, ScrollView, StyleSheet, Text, View, KeyboardAvoidingView, Platform } from 'react-native'
import {
...
const editFacilities = (props) => {
    ...
    const [facilitesRes,setFacilitesRes] = useState([]);
    ...
    const getListingDetail = () => {
        ...
        axios.get(`${Url}api/listing/by/${recordId}`, { headers: headers })
            .then(resp => {
                ...
                setFacilitesRes(response.facilites)
    ...
    const addFacilities = () => {
        if (facilitesRes.length == 0) {
            ...
            dispatch(editServiceFacilities(token, recordId, data, props.navigation, 1))
        }
    }
            ...
            <View style={styles.container} >
                ...
                <ScrollView contentContainerStyle={{ width: wp('100'), alignItems: 'center', paddingBottom: 20 }} >
                    <View style={styles.subContainer} >
                        ...
                        <CustomTitleDropdown item={Facilities} value={facilitesRes} label="What Facilities are used for this service?" placeHolder="Select Facilities" getValue={getFacilities.bind(this)} />
                        ...
                        <Btn label="Update & Continue" onClick={() => { addFacilities() }} bgColor="#2C99C6" />
                    </View>
                </ScrollView>
                {
                    loader
