...
const editFrequentlyQuestions = props => {
  ...
  const saveQuestion = () => {
    if (question == '') {
      showMessage('Question field should not be blank', 'warning');
    } else if (answer == '') {
      showMessage('Answer field should not be blank', 'warning');
    } else {
      ...
      dispatch(
        addFrequentlyAskedQuestions(token, recordId, data, props.navigation, 1),
      );
      setQuestion('');
      setAnswer('');
    }
      ...
      <ScrollView
        ...
        <View style={styles.subContainer}>
          ...
          <Text style={styles.subHeading}>
            Give Users more info by adding some FAQ's. Think about what
            additional info and details they would like to know, so they are
            comfortable booking now. Examples: What's included and/or provided,
            What they can expect to learn, How to find you, What to do on
            arrival, etc...
          </Text>
          ...
          <Btn
            label="Continue"
            onClick={() =>
              props.navigation.navigate('editCancellationPolicy', {
                recordId: recordId,
              })
            }
            bgColor="#2C99C6"
...
export default editFrequentlyQuestions;
