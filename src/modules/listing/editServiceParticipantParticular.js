...
const editServiceParticular = props => {
  ...
  const [responseSelectGoodFor, responsSetGoodFor] = useState([]);
  ...
  const uploadServiceParticulars = () => {
    if (gender == '') {
      showMessage('Should select geneder', 'warning');
    ...
    } else if (responseSelectGoodFor.length == 0) {
  ...
  const getData = () => {
    try {
      ...
      responsSetGoodFor(data.goodfor);
