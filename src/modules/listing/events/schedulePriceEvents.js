...
const schedulePrice = (props) => {
    ...
    const getStartDate = value => {
        ...
        if (y < x) {
            showMessage("Start date should not less than current date", 'warning')
    ...
    const getEndDate = value => {
        ...
        const y = new Date(value).setHours(0, 0, 0, 0);
        if (y < x) {
            showMessage("End date should not less than start date", 'warning')
    ...
    const getAccpetingBookingUntil = (value) => {
        ...
        if (compareDate <= EDate && compareDate >= SDate) {
            ...
            showMessage("Accepting booking until should be between start and end date", 'warning')
        }
    };
    ...
    const saveSchedulePrice = () => {
        ...
        if (serviceName == '') {
            showMessage('Schedule title field should not be blank', 'warning');
        } else if (se < ss) {
            showMessage("End date should not be smaller then start date", 'warning')
        } else if (seu < ss || seu > se) {
            showMessage("Accepting bookings until date should between start and end date", 'warning')
        } else if (repeatRadio == 'Yes' && (ends < ss || ends > se)) {
            showMessage("Repeat ends date should between start and end date", 'warning')
        } else if (quantity == '') {
            showMessage('Pricing quantity field should not be blank', 'warning');
        } else if (price == '') {
            showMessage('Price field should not be blank', 'warning');
        } else if (pricePer == '') {
            showMessage('Should select pricing per type', 'warning');
        } else {
    ...
    const continueClick = () => {
        if (scheduleList.length == 0) {
            ...
            props.navigation.navigate("frequentlyQuestionsEvent", { recordId: recordId })
        }
    }
    ...
    return (
        <View style={styles.container} >
            ...
            <ScrollView
                ref={scrollRef}
                contentContainerStyle={{ width: wp('100'), alignItems: 'center', paddingBottom: 20 }}
            >

                <View style={styles.subContainer} >
                    <Text style={styles.headingStyle} >Schedule and Price</Text>
                    <Text style={styles.subHeading} >Enter the dates, days, times, and other particulars for your event</Text>
                    <Text style={styles.subHeading1} >You have the option to add multiple schedules for your event, using different dates, days, and times</Text>
                    <Text style={styles.subHeading1} >Note: If your event has multiple locations, then a new listing must be created for each location</Text>
                    ...
                    <CustomInputField
                        ...
                        subLabel="Add a Sub Title or Name for this Schedule, (examples: 2nd Division, Over 30's, 5K Race)"
                    ...
                    <View style={{ width: wp('80'), marginTop: 12 }} >
                        ...
                        <Text style={[styles.subHeading1, { fontSize: wp('3') }]} >This is the date that your event starts</Text>
                        <CustomDatePicker value={startDate} getValue={getStartDate.bind(this)} />
                    </View>
                    <View style={{ width: wp('80'), marginTop: 12 }} >
                        <Text style={styles.labelStyle} >End date</Text>
                        <Text style={[styles.subHeading1, { fontSize: wp('3') }]} >This is the date that your event ends</Text>
                        ...
                        repeatRadio == "No"
                            ...
                            <>
                                <View style={{ width: wp('80'), marginTop: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }} >
                                    ...
                                    <View>
                                        <Text style={[styles.labelStyle, { width: wp('37.5'), marginTop: 5 }]} ></Text>
