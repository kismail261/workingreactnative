...
const editFacilityFAQ = props => {
  ...
  const saveQuestion = () => {
    if (question == '') {
      showMessage('Question field should not be blank');
    } else if (answer == '') {
      showMessage('Answer field should not be blank');
    } else {
      ...
      dispatch(
        addFrequentlyAskedQuestions(token, recordId, data, props.navigation, 3),
      );
      setQuestion('');
      setAnswer('');
    }
...
export default editFacilityFAQ;
