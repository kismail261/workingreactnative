...
const editFacilityLocation = props => {
  ...
  const saveLocation = () => {
    if (address == '') {
      ...
      dispatch(addServiceLocation(token, recordId, data, props.navigation, 3));
    }
  };
    ...
    try {
      if (address == '') {
        ...
        if (response.status === 200 || response.status === 201) {
          props.navigation.navigate('editFacilityFacilities');
        }
      }
    } catch (error) {
      console.error(error);
...
export default editFacilityLocation;
