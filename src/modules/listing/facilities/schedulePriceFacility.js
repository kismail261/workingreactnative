...
const schedulePrice = (props) => {
    ...
    const getStartDate = value => {
        ...
        if (y < x) {
            showMessage("Start date should not less than current date")
    ...
    const getEndDate = value => {
        ...
        const y = new Date(value).setHours(0, 0, 0, 0);
        if (y < x) {
            showMessage("End date should not less than start date")
    ...
    const getAccpetingBookingUntil = (value) => {
        ...
        if (compareDate <= EDate && compareDate >= SDate) {
            ...
            showMessage("Accepting booking until should be between start and end date")
        }
    };
    ...
    const saveSchedulePrice = () => {
        ...
        if (serviceName == '') {
            showMessage('Schedule title field should not be blank');
        } else if (se < ss) {
            showMessage("End date should not be smaller then start date")
        } else if (seu < ss || seu > se) {
            showMessage("Accepting bookings until date should between start and end date")
        } else if (repeatRadio == 'Yes' && (ends < ss || ends > se)) {
            showMessage("Repeat ends date should between start and end date")
        } else if (quantity == '') {
            showMessage('Pricing quantity field should not be blank');
        } else if (price == '') {
            showMessage('Price field should not be blank');
        } else if (pricePer == '') {
            showMessage('Should select pricing per type');
        } else {
    ...
    const continueClick = () => {
        if (scheduleList.length == 0) {
            ...
            props.navigation.navigate("InsuranceRequirements", { recordId: recordId })
        }
    }
    ...
    return (
        <KeyboardAvoidingView
            ...
            <View style={styles.container} >
                ...
                <ScrollView ref={scrollRef} contentContainerStyle={{ width: wp('100'), alignItems: 'center', paddingBottom: 20 }} >
