...
const addParticulars = (props) => {
    ...
    const getToDate = (value) => {
        if (fromDate > value) {
            showMessage("Age To value should be greater than Age From", 'warning');
            setToDate(value);
        } else {
            setToDate(value);
        }
    ...
    const uploadServiceParticulars = () => {
        if (gender == 'Select') {
            ...
            showMessage("Should select Age From", 'warning')
        } else if (toDate == 0) {
            showMessage("Should select Age To", 'warning')
            ...
            dispatch(addServiceParticulars(token, recordId, data, props.navigation, 1))
        }
    }
    ...
    return (
        <KeyboardAvoidingView
            ...
            <View style={styles.container} >
                <Header navigation={props.navigation} label="Service" progressCount={progress} onClose={onCloseHeader} />
                <ScrollView contentContainerStyle={{ width: wp('100'), alignItems: 'center', paddingBottom: 20 }} >
                    <View style={styles.subContainer} >
                        ...
                        <Text style={styles.subHeading} >Add some particulars to describe who the service is most suited to, and help Users choose what's best for them.</Text>
                        <View>
                            <Text style={[styles.labelStyle, { width: wp('80') }]} >Gender</Text>
                            <View style={{ width: wp('80'), marginTop: 0 }}>
                                <Dropdown
                                    name={gender}
                                    data={['Male', 'Female', 'Others', 'All']}
