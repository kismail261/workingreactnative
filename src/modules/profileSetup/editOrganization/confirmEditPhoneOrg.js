...
const confirmEditPhoneOrg = ({navigation, route}) => {
  ...
  const handlerOnFulfill = useCallback(async verificationCode => {
    setLoader(true);
    let headers = {
    ...
    axios
      .get(`${Url}api/users/verifyphone/${verificationCode}`, {
        headers: headers,
      })
      .then(resp => {
        ...
        setLoader(false);
        if (response) {
          navigation.navigate('editOrgLocationIinfo');
        } else {
  ...
  const resendCode = () => {
    ...
    axios
      ...
      .then(resp => {
        ...
        setLoader(false);
        showMessage(`code is sent on ${phoneNo}`);
      })
      .catch(error => {
        const err = error;
        if (err.response) {
      ...
      <ScrollView
        ...
        <View
          ...
          <Text style={styles.subheadingStyle}>
            Enter the four-digit code we sent to your phoneto confirm your
            number
          </Text>
        </View>
...
export default confirmEditPhoneOrg;
