...
const editOrgLocationIinfo = (props) => {
    const dispatch = useDispatch()
    const loader = useSelector(state => state.loader.loader)
    const { currentUser, currentUser : { _id: currentUserID } } = useSelector(state => state.user);
    ...
    useEffect(() => {
        ...
        dispatch(getLocationsList(currentUserID))
        return () => {
    ...
    const saveLocation = () => {
        if (address == '') {
            showMessage("Should choose location");
        } else {
            dispatch(setLoader(true))
            let data = {
                "location": [
            ...
            props.navigation.navigate("editOrgHomeMain")
        }
    }
...
export default editOrgLocationIinfo
