...
const EditContactIinfo = props => {
      ...
      try {
        ...
        getAllCountries().then(countries => {
          const country = countries.find(
            c => c.callingCode[0] === data.phone.substr(1, 2),
          );
  ...
  const updateData = async () => {
    try {
      if (firstName === '') {
        showMessage('First name field should not be blank');
      } else if (lastName === '') {
        showMessage('Last name field should not be blank');
      } else if (code === '') {
        showMessage('Select country code');
      } else if (phone == '') {
        showMessage('Phone number field should not be blank');
      } else if (phone.length != 10) {
        showMessage('Phone number should be 10 digit');
      } else {
        ...
        axios
          .post('https://api2.sporforya.com/api/users/contactinfo', data, {
            headers: headers,
          })
          .then(resp => {
            ...
            props.navigation.navigate('confirmEditPhone', {
              phoneNo: '+' + code + phone,
            });
  ...
  const getFullName = value => {
    if (value.length < 20) {
      setFirstName(removeEmojis(value));
    }
  };
  const getLastName = value => {
    if (value.length < 20) {
      setLastName(removeEmojis(value));
    }
  };
