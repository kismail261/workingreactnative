...
import ProviderTypeDropdown from './components/providerTypeDropdownUpdate';
import { showMessage } from '../../../helpers/replaceDigitsWithAesteriks';

var photos = []
const editHomeMain = (props) => {
  ...
  const [bio, setBio] = useState(userDetail.providerInfo.bio)
  const [description, setDescription] = useState(userDetail.providerInfo.description)
  const [personalizedUrl, setPersonalizedUrl] = useState('')
  const [avatar, setAvatar] = useState('')
  const [avatarLocal, setAvatarLocal] = useState(userDetail.providerInfo.avatar)
  const [photosLocal, setPhotosLocal] = useState(userDetail.providerInfo.image)
    ...
    try {
      if (avatarLocal == '') {
        ...
        if (response.status == 201) {
          props.navigation.navigate("bottomTab", { screen: 'providerHome' });
        }
        dispatch(setLoader(false))
      }
    } catch (error) {
          ...
          <View style={{ width: wp('80%'), alignSelf: 'center' }}>
            ...
            <View style={styles.addDp}>
              <TouchableOpacity
                ...
                <Image source={avatarLocal == '' ? ICONS.userIcon : { uri: ImageUrl + avatarLocal }} style={{ height: wp('23'), width: wp('23'), borderRadius: wp('12') }} />
                <Image
                  source={ICONS.edit}
            ...
            <View style={{ marginVertical: 20 }}>
              <ButtonRegular onClick={updateData} blue title="Update & Next" />
            </View>
