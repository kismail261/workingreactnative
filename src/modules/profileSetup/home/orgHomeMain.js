...
import { saveOrgProviderInfo, } from '../../../redux/profile/profile.action'
...
const orgHomeMain = (props) => {
  ...
  const saveProvider = () => {
    if (selectProvider.length == 0) {
      ...
      dispatch(saveOrgProviderInfo(data, token, props.navigation, currentUser._id))
    }
          ...
          <View style={{ width: '80%', alignSelf: 'center' }}>
            ...
            <View style={styles.addDp}>
              <TouchableOpacity
                ...
                <Image source={avatarLocal == '' ? ICONS.orgIcon : { uri: avatarLocal }} style={{ height: wp('23'), width: wp('23'), borderRadius: wp('12') }} />
                <Image
