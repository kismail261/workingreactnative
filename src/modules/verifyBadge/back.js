...
const BackVerification = ({ navigation }) => {
  const camera = createRef()
  const takePicture = async () => {
    if (camera) {
      const options = { quality: 0.5, base64: false };
      const data = await camera.current.takePictureAsync(options);
      navigation.navigate('verifyUser', { type: 'back', imgData: data })
    }
  };
      ...
      <View  >
        ...
        <View style={{ width: '80%', marginTop: 10 }}>
          <Text style={[styles.descx]}>
            Make sure there's good lighting and adjust front of your ID fits within the frame then tap the camera icon
          </Text>
        </View>
      </View>
      <View
...
export default BackVerification
